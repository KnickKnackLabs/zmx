            ...
            if (revents & lib_posix.POLL.IN != 0) {
                ...
                while (client.read_buf.next()) |msg| {
                    switch (msg.header.tag) {
                        ...
                        .EnvSet => daemon.handleEnvSet(gpa, client, msg.payload) catch |err| switch (err) {
                            error.InvalidEnvName, error.InvalidEnvValue, error.InvalidEnvRecord => {
                                std.log.warn("rejecting invalid client environment", .{});
                                const last = daemon.closeClient(gpa, client, i, false);
                                if (last) break :daemon_loop;
                        ...
                        .Ack, .TaskComplete, .LabelData, .EnvData, .ControlReady, .ControlViewport, .ControlLive, .ControlHistoryChunk, .ControlHistoryEnd => {},
...
pub const Client = struct {
    ...
    fn setEnv(self: *Client, gpa: std.mem.Allocator, env_str: []const u8) !void {
        try tracked_env.validate(env_str);
        const owned = if (env_str.len > 0) try gpa.dupe(u8, env_str) else null;
        if (self.env_str) |s| gpa.free(s);
        self.env_str = owned;
    }
};
...
pub const Daemon = struct {
    ...
    pub fn handleControlInit(
        ...
        // As with native init, let the child's redraw follow the resized snapshot.
        var ws = resize.winsize();
        _ = cross.c.ioctl(pty_fd, cross.c.TIOCSWINSZ, &ws);
        client.has_pending_output = true;
...
test "control viewport is serialized after resizing and does not mark a native terminal" {
    const alloc = std.testing.allocator;
    ...
    defer stream.deinit();
    stream.nextSlice("A line that wraps only in the smaller viewport.\r\n");
    const resize = ipc.Resize{ .rows = 10, .cols = 20 };
    try daemon.handleControlInit(alloc, client, -1, &term, std.mem.asBytes(&resize));
    ...
    const expected = util.serializeViewportSnapshot(alloc, &term) orelse return error.TestUnexpectedNull;
    defer alloc.free(expected);
    try std.testing.expectEqualStrings(expected, viewport.payload);
...
test "handleEnvGet returns leader client's environment variables including unsets" {
    ...
    // Rejected updates cannot replace the leader's environment or be ACKed.
    client2.write_buf.clearRetainingCapacity();
    client2.has_pending_output = false;
    try std.testing.expectError(error.InvalidEnvName, daemon.handleEnvSet(alloc, &client2, "DISPLAY=:2\n-X;touch injected;#\n"));
    try std.testing.expectEqualStrings("DISPLAY=:99\n", client2.env_str.?);
    try std.testing.expectEqual(@as(usize, 0), client2.write_buf.items.len);
    try std.testing.expect(!client2.has_pending_output);

    try std.testing.expectError(error.OutOfMemory, client2.setEnv(std.testing.failing_allocator, "DISPLAY=:3\n"));
    try std.testing.expectEqualStrings("DISPLAY=:99\n", client2.env_str.?);
}
