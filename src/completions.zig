...
const bash_completions =
    ...
    \\
    \\  local commands="attach run send print write detach list kill history control get set clear print-env wait tail completions version help"
    \\
    ...
    \\
    \\  case "$prev" in
    \\    attach|run|send|print|write|kill|history|control|get|set|clear|print-env|wait|tail)
...
const zsh_completions =
    ...
    \\    args)
    \\      case $words[2] in
    \\        attach|a|kill|k|run|r|send|s|print|p|write|wr|history|control|get|g|set|clear|print-env|hi|wait|w|tail|t)
    \\          _zmx_sessions
    \\          ;;
    \\        completions|c)
...
const fish_completions =
    ...
    \\complete -c zmx -n "__fish_is_nth_token 2; and __fish_seen_subcommand_from a attach r run s send p print wr write hi history control g get se set cl clear print-env" -a '(zmx list --short 2>/dev/null)' -d 'Session name'
...
const nu_completions =
    ...
    \\export extern "zmx list" [--short, --json]
